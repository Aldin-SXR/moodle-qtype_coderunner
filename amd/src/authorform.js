...
// jQuery is only used to listen for 'change' on the UI parameters field: the
// Ace UI signals edits with a jQuery-triggered event, which native listeners
// do not receive.
define(['jquery', 'qtype_coderunner/userinterfacewrapper', 'core/str'], function($, ui, str) {
    ...
    function initEditForm() {
        var typeCombo = document.getElementById('id_coderunnertype'),
            ...
            answerTextarea = document.getElementById('id_answer'),
        ...
        function getPrototype(qtype) {
            if (!prototypeRequests[qtype]) {
                ...
                url.searchParams.set('qtype', qtype);
                url.searchParams.set('courseid', courseId);
                url.searchParams.set('sesskey', M.cfg.sesskey);
                prototypeRequests[qtype] = fetch(url)
                    .then(response => response.json())
                    .then(outcome => (outcome && outcome.success ? outcome : null))
                    .catch(() => null);
            }
        ...
        function refreshUiParams(force) {
            ...
            const selected = typeCombo.options[typeCombo.selectedIndex];
            const qtype = selected ? selected.text : '';
            ...
            prototype.then(function(proto) {
                ...
                const ownParams = parseJsonObject(uiparameters.value);
                const protoParams = proto ? parseJsonObject(proto.uiparameters) : null;
                let merged = null;
                if (ownParams !== null && protoParams !== null) {
                    const sameUi = (proto.uiplugin || '').toLowerCase() === (uiplugin.value || '').toLowerCase();
                ...
                if (answerTextarea && json !== null && json !== answerTextarea.getAttribute('data-params')) {
                    ...
                    const preload = document.getElementById('id_answerpreload');
                    if (preload) {
                        preload.setAttribute('data-params', json);
                    }
                } else if (!force) {
                    return null;
                }
        ...
        var observer = new MutationObserver(function() {
            if (preloadHdr.classList.contains('collapsed')) {
                return;
            }
            ...
            if (preloadWrapper && typeof preloadWrapper.checkForResize === 'function') {
                ...
                setUi('id_answerpreload', uiplugin.value);
            }
        });
        if (preloadHdr) {
